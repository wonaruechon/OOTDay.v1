...
**Quick Start:** Copy each prompt below and paste directly into v0, Lovable, Cursor, or any AI coding tool.

---

## Prompt 1: Application Header Navigation
...
## Prompt 3: Filter Panel Component
...
3. Define 27 table columns in this specific order:
...
## Prompt 5: Pagination Controls Component
...
## Prompt 6: Confirmation Dialog Component

```
# HIGH-LEVEL GOAL
Create a reusable confirmation dialog component for bulk actions (like Reset Error) that follows Manhattan Active™ design patterns with proper modal overlay, title, message, and action buttons.
...
**Constraints:**
- Use MUI Dialog, DialogTitle, DialogContent, DialogActions components
- Dialog maxWidth prop set to "sm" (600px)
- Close icon uses MUI IconButton with CloseIcon from @mui/icons-material
- DO NOT implement actual API calls for actions; just call callback props
- Dialog should use controlled component pattern (open prop controls visibility)
...
**Files NOT to Modify:**
- Do NOT implement the API integration
- Do NOT create global state management for dialogs

**Expected Output:**
A fully functional, reusable confirmation dialog component that can be imported and used throughout the application. The dialog should handle its own close animations but delegate actions to parent components via callbacks. All dialogs should have polished styling, animations, and accessibility support.
```

---

## Prompt 7: Settings Dialog Component

```
# HIGH-LEVEL GOAL
Create a user settings dialog component with tabs for preferences (page size, default view, language) that saves settings to local storage.
...
**View Options (same as filter panel):**
```typescript
const viewOptions = [
  'Select an option',
...
**Constraints:**
- Use MUI Dialog, Tabs, Tab, Select, RadioGroup, Radio components
- Settings stored in localStorage (key: 'supplyManagementSettings')
- Profile tab should be visible but disabled/grayed out (future enhancement)
- Theme options should be visible but disabled (future enhancement)
...
**Files NOT to Modify:**
- Do NOT implement API integration for user profiles
- Do NOT create global settings context (parent handles localStorage)

**Expected Output:**
A fully functional settings dialog with working tabs, form inputs, and local state management. The component should call the onSave callback with validated settings object, and parent component handles persistence to localStorage.
```

---

## Prompt 8: Bookmark Management Component
...
5. Style the dropdown:
   ...
   - Padding: 16px
   - Border radius: 4px (desktop)
   - Shadow: MUI elevation 8
6. Implement "Save Current View" flow:
   - Clicking button opens BookmarkSaveDialog component (create separate dialog)
...
7. Manage bookmark state:
   ...
   - Parent component handles persistence (localStorage)
...
const defaultBookmarks: Bookmark[] = [
  ...
  },
  {
    id: 'default-errors',
    name: 'Error Items',
    filters: { includeErrored: 'yes', /* others empty */ },
    isDefault: true,
    createdAt: new Date().toISOString(),
  },
];
...
- Maximum 10 user bookmarks (plus 2 default = 12 total)
- Bookmarks sorted: Defaults first, then user bookmarks by createdAt desc
- Delete confirmation should use ConfirmationDialog component

# DEFINE STRICT SCOPE

**Files to Create:**
- `src/components/BookmarkDropdown.tsx`
- `src/components/BookmarkSaveDialog.tsx` (simple dialog with text input for bookmark name)

**Files to Import:**
- `src/components/ConfirmationDialog.tsx` (created earlier)

**Files NOT to Modify:**
- Do NOT create bookmark persistence logic; parent handles localStorage

**Expected Output:**
A fully functional bookmark dropdown component that integrates with the header, supports saving, loading, and deleting bookmarks, with proper hover states, animations, and accessibility.
```

---

## Manhattan Active™ Theme Setup (Required First)
...
1. Theme setup (copy theme.ts above)
...
7. Settings Dialog Component
8. Bookmark Management Component
9. Supply Details Dashboard (integrates all components)

---

**Created by:** Sally 🎨 (UX Expert)
**Date:** October 7, 2025
**Ready for:** v0, Lovable, Cursor, or any AI coding tool
